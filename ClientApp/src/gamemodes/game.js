...
export default class Game {
  ...
  createScene(engine, canvas) {
      ...
      assetsManager.addMeshTask("TrainingArenaTask", "", "./resources/levels/", "training.babylon")
      ...
      setTimeout(() => {
        ...
        readAllBytesAsUInt8Array("resources/levels/training_NavMesh.bin").then(d => {
            console.log("Array: ",d);
        ...
        // this.createAndDownloadNavMesh("training_NavMesh",navigationPlugin,this.navMeshes);
      },10);
  ...
  onArenaLoaded(newMeshes, particleSystems, skeletons) {
    ...
    newMeshes.forEach(m =>{
      ...
      m.position.addInPlaceFromFloats(0,2.8,0);
    });
  ...
  addBot() {
    this.bots.push(new Bot(this.scene,"Bocik"+this.bots.length+1,this.total, this));
  }
  ...
  createConnection(scene, serverIP, authSessionID) {
    const MESSAGE_ENUM = Object.freeze({
        ...
        ws.onmessage = evt => {
          ...
          switch (msg.type) {
            ...
            case MESSAGE_ENUM.SERVER_MESSAGE:
              ...
              if (msg.body.status) {
                if (this.status.timeLeft > 0 && msg.body.status.timeLeft <= 0) {
                  this.GUI.pushMessage("SERVER",`Game has ended. `, {color: "red", outlineWidth: 1, fontWeight: "bold"}, {color: "red", outlineWidth: 1});
                }
                if (this.status.started == false && msg.body.status.started == true) {
                  this.GUI.pushMessage("SERVER",`Game has started. `, {color: "red", outlineWidth: 1, fontWeight: "bold"}, {color: "green", outlineWidth: 1});
                }
            ...
            case MESSAGE_ENUM.SELF_CONNECTED:
              ...
              this.GUI.pushMessage("SERVER",`Welcome ${msg.body.name} on the server.`, {color: "red", outlineWidth: 1, fontWeight: "bold"}, {color: "green", outlineWidth: 1});
              const you = scene.getCameraByName("UniversalCamera");
      ...
      ws.onclose = evt => {
        ...
        this.GUI.pushMessage("SERVER","Connection with server lost. Game will continue in Single Player Mode.", {color: "red", outlineWidth: 1, fontWeight: "bold"});
        this.GUI.pushMessage("SERVER","J - Make bots move, K - Add Bot, L - Remove Bot", {color: "red", outlineWidth: 1, fontWeight: "bold"});
        setInterval(() => this.GUI.updateTime(this.status.timeLeft++),1000);
      }
