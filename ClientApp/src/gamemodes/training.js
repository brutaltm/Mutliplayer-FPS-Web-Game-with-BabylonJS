...
export default class Training {
  ...
  createScene(engine, canvas) {
      ...
      assetsManager.onFinish = tasks => {
          ...
          setTimeout(() => {
			...
			for (var i=0; i<3; i++) {
				this.bots.forEach(b => {
					var randomPointAround = this.navigationPlugin.getRandomPointAround(b.gracz.position, 60);
  ...
  onArenaLoaded(newMeshes, particleSystems, skeletons) {
    ...
    // const ground = BABYLON.Mesh.CreateGround("ground", 100, 100, 2, this.scene);
    // ground.checkCollisions = true;
    // ground.position.y -= 0.95;
    // ground.physicsImpostor = new BABYLON.PhysicsImpostor(ground, BABYLON.PhysicsImpostor.BoxImpostor, { mass: 0, restitution: 0.9, friction: 5 }, this.scene);
  ...
  setUpTraining() {
    ...
    // }
    this.GUI.pushMessage("TRAINING","You are playing Single Player Training.", {color: "red", outlineWidth: 1, fontWeight: "bold"});
    this.GUI.pushMessage("TRAINING","J - Make bots move, K - Add Bot, L - Remove Bot", {color: "red", outlineWidth: 1, fontWeight: "bold"});
